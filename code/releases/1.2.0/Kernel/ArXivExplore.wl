...
dirDatabase=Which[
				...
				developmentVersionQ&&$Notebooks,
				FileNameJoin[{ParentDirectory@NotebookDirectory[],"Assets"}]];


datasetLocal=
	Join@@Map[
		Import[FileNameJoin[{dirDatabase,"arxiv-database"<>"-"<>dateDataset<>"-"<>ToString[#]<>".json"}],"JSON"]&,Range[10]];
...
dateObjects:=dateObjects=Map[DateObject,dateStrings];


datasetDateObjects[All]:=datasetDateObjects[All]=
	KeySort@Merge[Rule@@@Transpose[{dateObjects,ArXivDataset[All]}],Identity];
arXivDatasetDated[All]["Day"]:=datasetDateObjects[All];
arXivDatasetDated[All]["Month"]:=arXivDatasetDated[All]["Month"]=
	Map[Flatten[#,1]&,Merge[KeyValueMap[DateObject@Drop[First[#1],-1]->#2&,datasetDateObjects[All]],Identity]];
arXivDatasetDated[All]["Year"]:=arXivDatasetDated[All]["Year"]=
	Map[Flatten[#,1]&,Merge[KeyValueMap[DateObject@Drop[First[#1],-2]->#2&,datasetDateObjects[All]],Identity]];
...
datasetKeys:=datasetKeys=Flatten@Union[Keys/@ArXivDataset[All]];
...
arXivDatasetItem:=arXivDatasetItem=AssociationThread[arXivIDsAssoc[All],ArXivDataset[All]];
...
categoriesUnion:=categoriesUnion=Union@Flatten[ArXivDatasetLookup["categories"]];

arXivCategoriesUnion:=arXivCategoriesUnion=Union[Flatten[StringSplit/@categoriesUnion]];

ArXivCategoriesUnion:=arXivCategoriesUnion;

ArXivCategoriesLegend:=ArXivCategoriesLegend=AssociationThread[arXivCategoriesUnion,
...
dateDays:=dateDays=dateStrings;


dateMonths:=dateMonths=Map[Take[#,2]&,dateStrings];


dateYears:=dateYears=Map[Take[#,1]&,dateStrings];
...
ArXivVersions[All]:=ArXivVersions[All]=arXivVersionsAssoc[All];
...
textWords=StringSplit[#,WordBoundary]&;
...
Options[ArXivVocabularyTitles]={"Stopwords"->False,"Dollar"->True};
...
Options[ArXivVocabularyAbstracts]={"Stopwords"->False,"Dollar"->True};
...
Options[ArXivVocabularyIntroductions]={"Stopwords"->False,"Dollar"->True};
	 ...
	 NetGraph @
		FunctionLayer[
			Module[{emb1, emb2, posembed},
				...
				posembed = SequenceIndicesLayer[OptionValue["SequenceIndicesLayerArgument"]][#Input];
...
selfAttentionBlock[depth_, heads_,drop_:0.5] := NetInitialize[
	NetGraph[
		FunctionLayer[
			Module[{keys, queries, values, seq},
			keys = LinearLayer[{heads, Floor[depth / heads]}] /@ #Input;
			values = LinearLayer[{heads, Floor[depth / heads]}] /@ #Input;
			queries = LinearLayer[{heads, Floor[depth / heads]}] /@ #Input;
			queries = queries / Sqrt[Floor[depth / heads]];
			seq = AttentionLayer["Dot", "MultiHead" -> True, "Mask" -> "Causal"][{keys, values, queries}];
			seq = LinearLayer[depth] /@ seq;
			seq = DropoutLayer[drop][seq];
			seq = seq + #Input;
...
feedForwardBlock[depth_,heads_] := NetInitialize[
	NetGraph[
		FunctionLayer[
			Module[{seq},
				seq = LinearLayer[heads * depth] /@ #Input;
...
Options[ArXivClassifyCategoriesNet]=Options[arXivClassifyNet];
...
ArXivClassifyCategoriesNet[catL_List,dim_List,drop_Real,idL___List,options:OptionsPattern[ArXivClassifyCategoriesNet]]:=
	ArXivClassifyCategoriesNet[catL,dim,drop,idL,options]=
			arXivClassifyNet[catL,catL,dim,drop,idL,options] 
